...
#creer une clase pour gerer event
class CometFallEvent:
    #lors de chargement creer compteur
    ...
    def meteor_fall(self):
        ...
        for i in range(1,10):
            #apparaitre premiere boule de feu
            self.all_comets.add(Comet(self))

    def attempt_fall(self):
        #la jauge d'event est totalment chargé
        if self.is_full_loaded() and len(self.game.all_monsters)==0:
            print("plus de commet")
            self.meteor_fall()
            self.fall_mode=True #activer event


    def update_bar(self,surface):
        #ajouter pourcentage a la barre a la bar
        self.add_percent()
